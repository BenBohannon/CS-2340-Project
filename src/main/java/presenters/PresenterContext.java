 ...
 * Singleton object that is injected into each presenter
 * to allow facilitate simple functions such as Scene
 * swapping while using the same dependency injector.
 * The PresenterContext should receive as constructor
 * params the dependency injector bindings and Stage
 * from the entry point, which should make for easier testing.
 * A test entry point could simply supply a different module
 * to change the behavior of the program. An instance of
 * PresenterContext additionally binds itself to be injected
 * as a singleton into each {@link Presenter} it generates.
 * This is so that the same dependency injector configuration
 * can be used for every new Presenter generated for an fxml view.
 */
public class PresenterContext {
    ...
    /**
     * Creates a new {@link Scene} from the fxml file
     * indicated, and sets it as the active Scene in the
     * Stage. If a {@link Presenter} is defined in the FXML,
     * the Presenter is created by the dependency injector
     * and returned. If a View is designated, the injected
     * Presenter is given a reference to the {@link View}
     * and the presenter is returned
     *
     * The presenter is returned before its initialize()
     * method has been called. you much call it manually.
     *
     * @param fxmlFileName name of the desired fxml file,
     *                     relative to the resources/presenters
     *                     directory. For example: "home_screen.fxml"
     * @return The presenter defined in the fxml file,
     *          or in the View
     */
    public Presenter showScreenUninitialized(String fxmlFileName) {
        ...
        if (handler instanceof View) {
            //If fxml designated a view,
            // give injected presenter a ref to the view
     ...
     * the active Scene in the Stage. If a {@link Presenter} is defined in the
     * FXML, thE Presenter is created by the dependency injector and returned.
     * If a View is designated, the injected Presenter is given a reference to
     * the {@link View} and the presenter is returned. Before it is returned,
     * the presenter's initialize() method is called.
     * @param fxmlFileName name of the desired fxml file, relative to the
     *                     resources/presenters directory. For example:
     *                     "home_screen.fxml"
